...
public class UserController implements Initializable {
    ...
    public ObservableList<Notification> getNotificationData(){
        ...
        try {
            ...
            rs = st.executeQuery();
            if (rs.next()) {
                Timestamp lastOpenTimestamp = rs.getTimestamp("last_open");
                LocalDateTime lastOpen = lastOpenTimestamp.toLocalDateTime();

                DayOfWeek dayOfWeek = lastOpen.getDayOfWeek();
                int dayValue = dayOfWeek.getValue();

                String querySchedule = "SELECT S.anime_id, TL.tl_id " +
                        "FROM Schedule S " +
                        "INNER JOIN TrackingAnime TA ON S.anime_id = TA.anime_id " +
                        "INNER JOIN TrackingList TL ON TA.tl_id = TL.tl_id " +
                        "WHERE TL.account_id = ? AND S.day >= ?";
                st = cnn.prepareStatement(querySchedule);
                st.setInt(1, data.accountid);
                st.setInt(2, dayValue);
                rs = st.executeQuery();
                while (rs.next()) {
                    int animeID = rs.getInt("anime_id");
                    int tlID = rs.getInt("tl_id");

                    String queryLastNotification = "SELECT MAX(time) AS last_time FROM Notification " +
                            "WHERE ref_tl_id= ? AND ref_anime_id= ?";
                    st = cnn.prepareStatement(queryLastNotification);
                    st.setInt(1, tlID);
                    st.setInt(2, animeID);
                    rs = st.executeQuery();
                    Timestamp lastNotificationTimestamp = null;
                    while (rs.next()) {
                        lastNotificationTimestamp = rs.getTimestamp("last_time");
                    }
                    boolean shouldInsertNotification = true;
                    if (lastNotificationTimestamp != null) {
                        LocalDateTime lastNotificationTime = lastNotificationTimestamp.toLocalDateTime();
                        if (lastNotificationTime.plusDays(7).isAfter(LocalDateTime.now())) {
                            shouldInsertNotification = false;
                        }
                    }
                    if (shouldInsertNotification) {
                        String insertNotification = "IF EXISTS (" +
                                "SELECT 1 FROM Schedule S " +
                                "INNER JOIN TrackingAnime TA ON S.anime_id = TA.anime_id " +
                                "WHERE TA.tl_id = ? AND TA.anime_id = ? AND S.day >= ?) " +
                                "BEGIN " +
                                "INSERT INTO Notification (ref_tl_id, ref_anime_id, ta_id, time) " +
                                "VALUES (?, ?, 1, CURRENT_TIMESTAMP); " +
                                "END";
                            st = cnn.prepareStatement(insertNotification);
                            st.setInt(1, tlID);
                            st.setInt(2, animeID);
                            st.setInt(3, dayValue);
                            st.setInt(4, tlID);
                            st.setInt(5, animeID);
            ...
            // Done
                    String query = " Select N.id,Acc.*,TL.tl_id,A.anime_id,A.poster,A.title,TA.status,N.time from Notification N " +
                            ...
                            "JOIN Account Acc ON TL.account_id = Acc.account_id where Acc.account_id= '" + data.accountid + "'";
